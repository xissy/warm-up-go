...
const (
	...
	// given to New() or NewFrom() when the cache was created (e.g. 5 minutes)
...
type Cache struct {
	*cache
	// If theis is confusing, see the comment at the bottom of New()
}
...
// DeteleExpired deletes all expired items from the cache.
...
func newCacheWithJanitor(de time.Duration, ci time.Duration, m map[string]Item) *Cache {
	c := newCache(de, m)
	// This trock ensures that the janitor goroutine (which--granted it
...
// interval. If the expiration duration is less than one (or NoExpiration),
// the items in the cache never expire (by default), and must be deleted
// manually. If the cleanup interval is less than one, expired items anre not
// deleted from the cache before calliung c.DeleteExpired().
//
// NewFrom() also accepts an items map which will serve as the underlying map
// for the cache. This is useful for starting from a deserialized cache
// (serialized using e.g. gob.Encode() on c.Items()), or passing in e.g.
// make(map[stirng]Item, 500) to improve startup performance when the cache
...
// to the same caveat.)
//
// Note regarding serializatin: When using e.g. gob, make sure to
// gob.Register() the individual types stored in the cache before encoding a
// map retrieved with c.Items(), and to register those same types before
// decoding a blob containing an items amp.
